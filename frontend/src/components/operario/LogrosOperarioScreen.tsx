  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="rounded-2xl bg-[var(--superficie)] p-4 shadow-sm">
        <p className="mb-2 text-xl font-medium text-[var(--texto)]">
          {desbloqueados}{" "}
          <span className="text-sm font-normal text-[var(--texto-secundario)]">/ {total} desbloqueados</span>
        </p>
        <div className="h-2 overflow-hidden rounded-full bg-[var(--superficie-alt)]">
          <div
...
function TarjetaLogro({ logro }: { logro: LogroResuelto }) {
  if (!logro.desbloqueado) {
    return (
      <div className="rounded-xl bg-[var(--superficie)] p-3 opacity-55 shadow-sm">
        <div className="mb-2 flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--superficie-alt)] text-lg grayscale">
          {logro.icono ?? <Lock size={18} className="text-[var(--texto-tenue)]" aria-hidden />}
        </div>
        <p className="mb-1 text-xs font-medium text-[var(--texto)]">{logro.nombre}</p>
        <p className="text-xs text-[var(--texto-tenue)]">???</p>
      </div>
    );
  }

  return (
    <div className="rounded-xl border border-emerald-400 bg-[var(--superficie)] p-3 shadow-sm">
      ...
      <p className="mb-1 text-xs font-medium text-[var(--texto)]">{logro.nombre}</p>
      {logro.descripcion && (
        <p className="mb-2 text-xs leading-relaxed text-[var(--texto-secundario)]">{logro.descripcion}</p>
      )}

      {logro.progreso ? (
        <>
          <div className="mb-1 flex items-center justify-between">
            <span className="text-xs font-medium text-[var(--acento)]">×{logro.veces}</span>
            <span className="text-[10px] text-[var(--texto-tenue)]">
              {logro.progreso.actual.toLocaleString("es-ES")} / {logro.progreso.objetivo.toLocaleString("es-ES")}
            </span>
          </div>
          <div className="h-1.5 overflow-hidden rounded-full bg-[var(--superficie-alt)]">
            <div className="h-full rounded-full bg-emerald-600" style={{ width: `${logro.progreso.pct}%` }} />
          </div>
        </>
      ) : (
        <span className="text-xs font-medium text-[var(--acento)]">
